...
**Bearlabs** · 从二进制到人工智能：**11 门课程 · 116 讲 · 双讲解 · 三级习题 · 220 道必刷题 · 16 个可动手交互动画**的学习网站。
...
- **必刷题（纯刷题板块）**：220 道计算机面试与 408 考研经典题（数据结构 / 组成原理 / 操作系统 / 网络 / 编程语言 / 算法 / 数据库 / 智力场景八大科目，7 种题型、三级难度递增）；每题「专业版 + 宝宝巴士版」双解析，支持按科目 / 题型 / 难度 / 关键词筛选，顺序刷、随机一题、错题重刷与收藏，做题记录本地持久化、刷新不丢。
...
│  ├─ problems/<科目>.json             # 必刷题题库（220 题，含双版本解析）
│  ├─ problems/manifest.json           # 题目索引（构建生成，勿手改）
...
- **必刷题**（#/problems）：纯刷题板块；按科目 / 题型 / 难度 / 关键词筛选，顺序刷、随机一题、只看未做 / 做错；答错自动进错题本，可收藏与重做；统计页可看正确率与进度、导出记录。
...
- 默认所有学习数据保存在**本机浏览器** localStorage（课程进度键 `cslearn.progress.v1`；必刷题记录键 `cslearn.problems.v1`），不上传任何服务器；清理浏览器数据前请先在进度面板 / 必刷题统计页导出备份。
...
- **v1.3**（2026-09-22）：新增「必刷题」纯刷题板块——220 道计算机面试与 408 经典题（8 科 × 7 题型 × 三档难度），每题双版本解析，含筛选、错题本、收藏与进度统计。
...
- 「必刷题」中的简答 / 代码 / 复杂度分析题由学习者对照参考答案自评，不做自动判分。
